...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate Avellaneda-Stoikov market making parameters"
    )
    parser.add_argument(
        "ticker", nargs="?", default="PAXG", help="Ticker symbol (default: BTC)"
    )
    parser.add_argument(
        "--minutes",
        type=int,
...
async def _get_market_details_async(
    symbol: str,
    ...
    try:
        ...
        # Symbol not found
        print(
            f"Warning: Market details for {symbol} not found in API response. Using fallback."
        )
...
def load_and_resample_mid_price(csv_path):
    ...
    if df["datetime"].duplicated().any():
        print(
            "Warning: Duplicate timestamps found in price data. Keeping last entry for each."
        )
...
def prepare_calculation_windows(
    mid_price_df: pd.DataFrame,
    trades_df: pd.DataFrame,
    window_minutes: int,
    freq_str: str,
    periods_to_keep: int = PERIODS_TO_USE,
    ...
    while True:
        ...
        mask = (mid_price_df.index >= last_period_start) & (
            mid_price_df.index < last_period_end
        )
    ...
    while True:
        ...
        mask = (mid_price_df.index >= first_period_start) & (
            mid_price_df.index < first_period_end
        )
        if mid_price_df.loc[mask].shape[0] < expected_points:
    ...
    if len(period_starts) > periods_to_keep:
        ...
        mid_price_df = mid_price_df.loc[
            (mid_price_df.index >= window_start) & (mid_price_df.index < window_end)
        ]
...
def calculate_garch_volatility(mid_price_df, window_minutes, freq_str):
    ...
    for i, period_start in enumerate(list_of_periods):
        ...
        returns = (
            historical_data["mid_price"].pct_change().dropna() * 100.0 * scale_fact
        )
        ...
        try:
            ...
            if i == len(list_of_periods) - 1:
                ...
                print(
                    f"  Persistence: {res.params['alpha[1]'] + res.params['beta[1]']:.6f}"
                )
...
def calculate_volatility(mid_price_df, window_minutes, freq_str):
    ...
    if num_periods < min_periods_for_garch:
        print(
            f"Fewer than {min_periods_for_garch} periods available, using rolling volatility only."
        )
        final_sigma = calculate_rolling_volatility(
            mid_price_df, window_minutes, freq_str
        )
    else:
        ...
        rolling_sigma = calculate_rolling_volatility(
            mid_price_df, window_minutes, freq_str
        )
            ...
            if pd.notna(g):
                ...
                if pd.notna(r) and abs(r) > 1e-12:
                    ...
                    if ratio > 5.0 or ratio < 0.2:
                        print(
                            f"  - Period {i}: GARCH sigma {g:.6f} deviates too much from rolling {r:.6f}. Using rolling value."
                        )
                        use_garch = False
                if use_garch:
...
def calculate_intensity_params(
    list_of_periods, window_minutes, buy_orders, sell_orders, deltalist, mid_price_df
):
    ...
    for i in range(len(list_of_periods)):
        ...
        mask_sell = (sell_orders.index >= period_start) & (
            sell_orders.index < period_end
        )
        ...
        best_bid = (
            period_buy_orders["price"].max() if not period_buy_orders.empty else np.nan
        )
        best_ask = (
            period_sell_orders["price"].min()
        ...
        if pd.isna(best_bid) or pd.isna(best_ask):
            ...
            reference_mid = (
                s_period["mid_price"].mean() if not s_period.empty else np.nan
            )
        else:
        ...
        for price_delta in deltalist:
            ...
            bid_hits = []
            if not period_sell_orders.empty:
                sell_hits_bid = period_sell_orders[
                    period_sell_orders["price"] <= limit_bid
                ]
            ...
            if not period_buy_orders.empty:
                buy_hits_ask = period_buy_orders[
                    period_buy_orders["price"] >= limit_ask
                ]
        ...
        lambdas = pd.DataFrame(
            {
                ...
                "lambda_delta": [
                    1 / d.mean() if len(d) > 0 else 1e-6 for d in deltadict.values()
                ],
            }
        ).set_index("delta")
...
def optimize_gamma(
    ...
    for j in period_index_range:
        ...
        if gamma_grid_to_test is None:
            gamma_grid_to_test = generate_gamma_grid(
                s.iloc[-1], sigma, k, window_minutes
            )
        ...
        sell_mask = (sell_trades.index >= period_start) & (
            sell_trades.index < period_end
        )
        ...
        for i, gamma_to_test in enumerate(gamma_grid_to_test):
            print(
                f"  - Testing gamma: {gamma_to_test:.5f} ({i+1}/{len(gamma_grid_to_test)})"
            )
        ...
        if valid_results.empty:
            print(
                "Warning: All backtests resulted in NaN or 0 PnL. Using fallback gamma."
            )
            best_gamma = 0.5
        else:
            positive_pnl_results = valid_results[valid_results["pnl"] > 0]
            if not positive_pnl_results.empty:
                best_gamma = positive_pnl_results.loc[
                    positive_pnl_results["spread"].idxmax()
                ]["gamma"]
            else:
...
def find_workable_spread(
    initial_spread, spread_func, k, direction="up", factor=1.05, max_iterations=100
):
...
def jit_backtest_loop(
    ...
    for i in range(N):
        ...
        sell = (
            1
            if not np.isnan(sell_max_values[i]) and sell_max_values[i] >= r_a[i]
            else 0
        )
        buy = (
            1 if not np.isnan(buy_min_values[i]) and buy_min_values[i] <= r_b[i] else 0
        )
        q[i + 1] = q[i] + (sell - buy)
...
def run_backtest(
    mid_prices, buy_trades, sell_trades, gamma, k, sigma, window_minutes, fee=0.00030
):
    ...
    buy_min = (
        buy_trades_clean["price"]
        .resample("5s")
        .min()
        .reindex(time_index, method="ffill")
    )
    sell_max = (
        sell_trades_clean["price"]
        .resample("5s")
        .max()
        .reindex(time_index, method="ffill")
    )
    ...
    spread_base = gamma * sigma**2.0 * time_remaining + (2.0 / gamma) * np.log(
        1.0 + (gamma / k)
    )
...
def print_summary(results, list_of_periods):
    ...
    if window_minutes is not None:
        if window_hours is not None:
            print(
                f"Analysis Period: {window_minutes} minutes ({window_hours:.2f} hours)"
            )
        else:
    ...
    print(f"Market Data:")
    print(
        f"   Mid Price:                        ${results['market_data']['mid_price']:,.4f}"
    )
    ...
    print(f"\nCurrent State:")
    print(
        f"   Time Remaining:        {results['current_state']['time_remaining']:.4f} (in days)"
    )
    print(f"   Inventory (q):         {results['current_state']['inventory']:.4f}")
    print(f"\nCalculated Prices:")
    print(
        f"   Reservation Price:     ${results['calculated_values']['reservation_price']:.4f}"
    )
    print(f"   Ask Price:             ${results['limit_orders']['ask_price']:.4f}")
    print(f"   Bid Price:             ${results['limit_orders']['bid_price']:.4f}")
    print(f"\nSpreads:")
    print(
        f"   Delta Ask:             ${results['limit_orders']['delta_a']:.6f} ({results['limit_orders']['delta_a_percent']:.6f}%)"
    )
    print(
        f"   Delta Bid:             ${results['limit_orders']['delta_b']:.6f} ({results['limit_orders']['delta_b_percent']:.6f}%)"
    )
    print(
        f"   Total Spread:          {(results['limit_orders']['delta_a_percent'] + results['limit_orders']['delta_b_percent']):.4f}%"
    )
...
def main():
    ...
    # TICKER = args.ticker
    # Use the MARKET_SYMBOL environment variable
    TICKER = os.getenv("MARKET_SYMBOL", "BTC")
    ...
    print(
        f"Using analysis period of {window_minutes} minutes ({window_hours:.2f} hours)."
    )
    if ma_window > 1:
    ...
    if market_id is not None:
        ...
        print(
            f"Could not fetch market details online. Using fallback price tick size: {price_tick_size}"
        )
    ...
    print(
        f"Loaded {len(mid_price_full_df)} data points from {mid_price_full_df.index.min()} to {mid_price_full_df.index.max()}."
    )

    freq_str = f"{window_minutes}min"
    calc_mid_price_df, calc_trades_df = prepare_calculation_windows(
    ...
    print(
        f"Using last {len(list_of_periods)} completed periods ({used_minutes} minutes) for parameter estimation."
    )

    # Calculate parameters
    sigma_list = calculate_volatility(calc_mid_price_df, window_minutes, freq_str)
    Alist, klist = calculate_intensity_params(
        list_of_periods,
        window_minutes,
        buy_trades,
        sell_trades,
    ...
    results = calculate_final_quotes(
        gamma, sigma, A, k, window_minutes, mid_price_full_df, ma_window
    )
